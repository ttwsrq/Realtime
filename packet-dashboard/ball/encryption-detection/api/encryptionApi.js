...
export async function logEncryptionDetection({ packetId, encryptionVersion, cipherSuite, handshakeTimeMs, isValid, notes }) {
  const { data, error } = await supabase
    ...
    .insert({ packet_id: packetId, encryption_version: encryptionVersion, cipher_suite: cipherSuite, handshake_time_ms: handshakeTimeMs, is_valid: isValid, notes })
    .select().single();
