// [คิว] Real-time Packet Capture API
...
export async function savePacket({ srcIp, dstIp, srcPort, dstPort, protocol, encryption, sizeBytes, payloadPreview }) {
  const { data, error } = await supabase
    .from("packets")
    .insert({ src_ip: srcIp, dst_ip: dstIp, src_port: srcPort, dst_port: dstPort, protocol, encryption, size_bytes: sizeBytes, payload_preview: payloadPreview, status: "captured" })
    .select().single();
...
export async function updatePacketStatus(id, status) {
  const { error } = await supabase
    .from("packets").update({ status, analyzed_at: new Date().toISOString() }).eq("id", id);
  if (error) throw error;
}
